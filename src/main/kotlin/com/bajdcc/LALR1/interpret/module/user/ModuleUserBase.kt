...
class ModuleUserBase : IInterpreterModule {
    ...
    companion object {
        ...
        private fun buildHandle(info: IRuntimeDebugInfo) {
            ...
            info.addExternalFunc("g_win_str",
                    RuntimeDebugExec("设置字符串", arrayOf(RuntimeObjectType.kPtr, RuntimeObjectType.kInt, RuntimeObjectType.kString))
                    { args: List<RuntimeObject>, status: IRuntimeStatus -> RuntimeObject(status.service.userService.window.str(args[0].int, args[1].int, args[2].string)) })
        }
