...
class SpringApiController {
    ...
    fun queryItem(@PathVariable(value = "item") item: String): Any {
        val obj = ModuleNet.instance.webApi?.sendRequest("query/$item")
        return if (obj != null)
    ...
    fun getMarkdown(@PathVariable(value = "item") item: String): Any {
        val obj = ModuleNet.instance.webApi?.sendRequest("md/$item")
        return if (obj != null)
            hashMapOf("code" to 200, "data" to obj)
        else
            hashMapOf("code" to 404)
    ...
    fun getVfs(@RequestParam("path") path: String): Any {
        val obj = ModuleNet.instance.webApi?.sendRequest(
    ...
    fun execCode(@RequestBody ctx: SpringBeanExec): Any {
        return if (ctx.code != null) {
            RuntimeProcess.writePipe(ModuleUserBase.EXEC_PREFIX + execId, ctx.code!!)
            val obj = ModuleNet.instance.webApi?.sendRequest("exec/${execId++}")
            if (obj != null)
                hashMapOf("code" to 200, "data" to obj)
            else
                hashMapOf("code" to 404)
    ...
    fun execQuery(@RequestParam("id") id: String): Any {
        val obj = ModuleNet.instance.webApi?.sendRequest("exec_query/$id")
        return if (obj != null)
            hashMapOf("code" to 200, "data" to obj)
        else
            hashMapOf("code" to 404)
    ...
    fun execKill(@RequestParam("id") id: String): Any {
        val obj = ModuleNet.instance.webApi?.sendRequest("exec_kill/$id")
        return if (obj != null)
            hashMapOf("code" to 200, "data" to obj)
        else
            hashMapOf("code" to 404)
